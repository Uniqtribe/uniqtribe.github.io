...
<div id="selectedColor" style='display:none'></div>
      ...
      loader.load(configObject.imageInfo.objectPath, function (gltf) {
         ...
         const {
            averagedColors,
            clusterVariances
         } = simpleClusterColors(palette, pixels);
         ...
         for (let i = recommendedPaletteCount; i <= maxPaletteCount; i++) {
            ...
            const {
               uniqueColoredPalettes,
               removedColoredPalettes
            } = removePalettesWithSimilarColors(uniquePalettes)
      ...
      buildCustomSelect({
         labelName: 'Shape',
         allowedValues: configObject.shapes,
         name: 'shape'
      });
...
function changeColor(changeColorArray) {
   ...
   const colorHexMap = basicColor[0].baseColor.map(({
         r,
         g,
         b
      }) =>
